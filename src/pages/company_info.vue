...
export default {
  ...
  created() {
    setTimeout(() => {
      if (this.token === '') {
        ...
        localstorageService.setRedirectUrl('company_info')
  ...
  methods: {
      ...
      this.$http
        ...
        .then(({ data }) => {
          if (data.StatusCode === 200) {
            localstorageService.setUserInfo(data.Data)
            ...
            if (data.Data.Other) {
              ...
              this.form.Introduction = other.Introduction
                ? other.Introduction
                : '' // 简介
    ...
    msgConfig() {
      ...
      this.$http
        .get('/api/wechat/config/share/by?url=' + paramsUrl)
        .then(doc => {
      ...
      this.$http.get('/api/exhibition', this.headers).then(({ data }) => {
        if (data.StatusCode === 200) {
          ...
          if (this.invite && this.invite !== '') {
            const selelcExhibition = data.Data.filter(
              v => v.Id === this.invite.ItemId
            )
      ...
      if (this.formValid(this.form)) {
        ...
        this.validPhone().then(cur => {
          ...
          const customerBody = {
            CompName: this.form.CompName,
            ShortName: this.form.ShortName,
            Logo: this.user.HeadImgUrl
              ? this.user.HeadImgUrl
              : 'http://api.auth.huizhanren.cn/upload/images/201804/0f145ddea71740d29f842bdd6d965e2d.jpg',
            Mob: this.form.Mob,
          ...
          const userinfo$ = this.$http
            .put('/api/userinfo', userinfoBody, this.headers)
            .then(({ data }) => {
              return data
            })
          const customer$ = this.$http
            .post('/api/customer', customerBody, this.headers)
            .then(({ data }) => {
              return data
            })
          // 1.先验证手机 2.新增展商 根据返回的customer的id新增作品 3.添加用户信息
          Promise.all([customer$, userinfo$])
            .then(result => {
              const compinfoBody = {
              ...
              this.$http
                .post('/api/compinfo', compinfoBody, this.headers)
                .then(({ data }) => {
    ...
    resubmit() {
      ...
      if (this.formValid(this.form)) {
        ...
        const compinfo$ = this.$http
          .post('/api/compinfo', compinfoBody, this.headers)
          .then(({ data }) => {
            return data
          })
        // 1.修改用户信息 2.新增作品
        Promise.all([compinfo$, userinfo$])
          .then(result => {
    ...
    validPhone() {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.$http
          .post('/api/basic/validate/sms/check', body, this.headers)
          .then(doc => {
            let state = doc.data
